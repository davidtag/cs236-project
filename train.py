...
# New args
parser.add_argument('--generator', type=str, default='baseline', choices=['baseline', 'glow'])
...
try:
    for epoch in range(opt.epoch, opt.n_epochs):
        for i, batch in enumerate(dataloader):
            ...
            A = batch['A'] * opt.scale_pixels
            B = batch['B'] * opt.scale_pixels
            ...
            # GAN loss
            fake_B = generator.transform(real_A) * opt.scale_pixels
            pred_fake = netD_B(fake_B)
            loss_GAN_A2B = criterion_GAN(pred_fake, target_real)

            fake_A = generator.transform.inv(real_B) * opt.scale_pixels
