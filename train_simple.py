if __name__ == '__main__':
    ...
    try:
        ...
        A = Image.open('datasets/horse2zebra/train/A/n02381460_1001.jpg') #('datasets/facades/train/A/103_A.jpg')
        B = Image.open('datasets/horse2zebra/train/B/n02391049_10007.jpg')


        #fig,((ax11,ax12,ax13),(ax21,ax22,ax23)) = plt.subplots(2,3,figsize=(15,10),sharex=True,sharey=True)
        #ax11.imshow(A)
        #ax12.imshow(B)
        #ax11.set_title('Real A')
        #ax12.set_title('Real B')
        #plt.show()

        im_A_arr = np.array(A)/256.0
        im_B_arr = np.array(B)/256.0
        ...
        for epoch in range(opt.epoch, opt.n_epochs):
            ...
            for i, batch in enumerate(range(10)):
                ...
                fake_B = generator.transform(real_A)
                loss_GAN_A2B = criterion_GAN(fake_B, real_B)
                ...
                #print("\n------done batch")
                if i % 4 == 0:
                    print("Generator loss: ", float(loss_GAN_A2B))
                    """
                    logger.log({'loss_G': loss_GAN_A2B, 
            ...
            if(epoch > 1 and epoch % 10 == 0):
                fake_B_copy = fake_B.permute(0,2,3,1)
                print(fake_B.shape)
                print(fake_B_copy.shape)
                fake_B_copy = fake_B_copy.detach().numpy()
                fake_B_copy = fake_B_copy[0,:,:,:]
                print(fake_B_copy.shape)
                ...
                ax11.imshow(A)
                ax12.imshow(B)
                ax13.imshow(fake_B_copy)

                ax11.set_title('Real A')
                ax12.set_title('Real B')
                ax13.set_title('Fake B')
                plt.show()
