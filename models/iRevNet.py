...
class irevnet_block(nn.Module):
    ...
    def forward(self, x):
        ...
        y1 = Fx2 + x1
        #y1 = x1 + torch.cat([Fx2]*self.stride**2,1)
        return (x2, y1)
...
class iRevNet(nn.Module):
    ...
    def forward(self, x):
        ...
        print(out_bij.shape)
        return self.reshape(out_bij,dims=(batch_in,channels_in,width_in,height_in),direction='forward')
        
    def inverse(self, out_bij):
        """ irevnet inverse """
        out_bij = self.reshape(out_bij)
        ######################################
        out = split(out_bij)
...
if __name__ == '__main__':
    ...
    print("y.shape = {}".format(x_rec.shape))
    print(((x-x_rec)**2).sum())
